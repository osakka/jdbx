...
int query_tracker_init(void) {
    ...
    if (!g_query_tracker) {
        LOG_ERROR("Failed to allocate query tracker");
        return 0;
    }
    ...
    if (!g_query_tracker->patterns) {
        ...
        LOG_ERROR("Failed to allocate query tracker hash table");
        return 0;
    }
    ...
    if (pthread_rwlock_init(&g_query_tracker->lock, NULL) != 0) {
        ...
        free(g_query_tracker);
        g_query_tracker = NULL;
        LOG_ERROR("Failed to initialize query tracker lock");
        return 0;
    }
...
static query_pattern_t* find_or_create_pattern(const char* collection_name, const char* field_path) {
    ...
    LOG_TRACE("Created new query pattern: %s.%s", collection_name, field_path);
    ...
    if (field_paths && field_count > 0) {
        ...
        for (size_t i = 0; i < field_count; i++) {
            ...
            if (pattern) {
                ...
                LOG_TRACE("Updated pattern %s.%s: count=%lu, avg=%.2fms", 
