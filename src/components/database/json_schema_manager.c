...
int db_store_json_schema(database_t* db, const char* collection_name, json_value_t* schema) {
    ...
    if (!schema_doc) {
        LOG_ERROR("Failed to create schema document");
        return 0;
    }
    ...
    if (!db_collection_exists(db, SCHEMAS_COLLECTION)) {
        if (!db_create_collection(db, SCHEMAS_COLLECTION)) {
            LOG_ERROR("Failed to create schemas collection");
    ...
    if (!result) {
        LOG_ERROR("Failed to store schema for collection: %s", collection_name);
        return 0;
    }
    
    json_free(result);
    LOG_INFO("Schema stored successfully for collection: %s", collection_name);
    return 1;
}
