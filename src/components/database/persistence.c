...
static void* persistence_thread_main(void* arg) {
  ...
  if (g_logger) LOG_DEBUG("Started: db=%p, path=%s", db, db->path);
...
int db_start_persistence_thread(database_t* db) {
  ...
  LOG_DEBUG("Starting for: %s", db->path);
  ...
  if (!db->persistence) {
    LOG_ERROR("initialize persistence structure");
    return 0;
  }
  ...
  if (pthread_create(&db->persistence->thread, NULL, persistence_thread_main, db) != 0) {
    if (g_logger) LOG_ERROR("create persistence thread: %s", strerror(errno));
    persistence_cleanup(db->persistence);
    db->persistence = NULL;
    return 0;
  }
  
  if (g_logger) LOG_DEBUG("Started for: %s", db->path);
  return 1;
}
...
void db_stop_persistence_thread(database_t* db) {
  ...
  if (g_logger) LOG_DEBUG("Stopping thread: %s", db->path);
...
int db_notify_data_change_sync(database_t* db, size_t estimated_size) {
  ...
  if (g_logger) LOG_DEBUG("Changes: ops=%d, size=%zu", 
