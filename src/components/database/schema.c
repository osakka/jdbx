...
schema_t* db_create_schema(const char* name, const char* description) {
  ...
  if (!name) {
    LOG_ERROR("create schema: name cannot be NULL");
    return NULL;
  }

  schema_t* schema = (schema_t*)malloc(sizeof(schema_t));
  if (!schema) {
    LOG_ERROR("Out of memory", name);
    return NULL;
  }
...
schema_rule_t* db_create_schema_rule(schema_rule_type_t type, const char* field_path) {
  ...
  if (!field_path) {
    LOG_ERROR("create schema rule: field_path cannot be NULL");
    return NULL;
  }
...
int db_add_schema_rule(schema_t* schema, schema_rule_t* rule) {
  if (!schema || !rule) {
    LOG_ERROR("add schema rule: NULL schema or rule");
    return 0;
  }
...
int db_attach_schema(database_t* db, const char* collection, schema_t* schema) {
  ...
  if (!coll) {
    LOG_ERROR("attach schema: collection '%s' not found", collection);
    return 0;
  }
...
int db_detach_schema(database_t* db, const char* collection) {
  ...
  db_collection_t* coll = db_get_collection(db, collection);
  if (!coll) {
    LOG_ERROR("detach schema: collection '%s' not found", collection);
    return 0;
  }
...
schema_t* db_get_schema(database_t* db, const char* collection) {
  ...
  db_collection_t* coll = db_get_collection(db, collection);
  if (!coll) {
    LOG_ERROR("get schema: collection '%s' not found", collection);
    return NULL;
  }
...
json_value_t* db_schema_to_json(schema_t* schema) {
  ...
  if (!schema) {
    LOG_ERROR("convert schema to JSON: NULL schema");
    return NULL;
  }
  ...
  if (!json) {
    LOG_ERROR("create JSON object for schema");
    return NULL;
  }
  ...
  while (rule) {
    ...
    if (!rule_obj) {
      LOG_ERROR("create JSON object for rule %d", rule_index);
...
schema_t* db_schema_from_json(json_value_t* json) {
  ...
  if (!json || json->type != JSON_OBJECT) {
    LOG_ERROR("create schema from JSON: invalid JSON or not an object");
    return NULL;
  }
  ...
  if (!name_val || name_val->type != JSON_STRING) {
    LOG_ERROR("create schema from JSON: missing or invalid 'name' property");
    return NULL;
  }
  ...
  if (!schema) {
    LOG_ERROR("create schema object");
    return NULL;
  }
  ...
  if (!rules_arr || rules_arr->type != JSON_ARRAY) {
    LOG_ERROR("get rules array from schema JSON");
  ...
  for (size_t i = 0; i < rule_count; i++) {
    ...
    if (!rule) {
      LOG_ERROR("create rule of type '%s' for field '%s'",
           type_val->value.string, field_val->value.string);
      continue;
    }
