        ...
        if (doc && doc->type == JSON_OBJECT) {
            ...
            if (field_val) {
                ...
                if (btree_disk_insert(index, field_str, strlen(field_str), offset) == 0) {
                    ...
                    LOG_WARNING("Failed to index document %s for field %s", doc_id, field_name);
                }
            }
            json_free(doc);
        }
