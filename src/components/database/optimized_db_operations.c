...
json_value_t* optimized_db_list_collections(database_t* db) {
    if (!db) {
        LOG_ERROR("Failed to list collections: NULL database");
        return NULL;
    }
    ...
    if (!result) {
        LOG_ERROR("Failed to create result array for collections");
        return NULL;
    }
    ...
    if (collection_count > 0) {
        ...
        if (!collection_names) {
            pthread_mutex_unlock(&db->lock);
            json_free(result);
            LOG_ERROR("Failed to allocate memory for collection names");
            return NULL;
        }
...
json_value_t* optimized_db_query_documents(database_t* db, const char* collection_name, json_value_t* query_json) {
    LOG_INFO("Starting optimized document query for collection '%s'", collection_name ? collection_name : "NULL");
    
    if (!db || !collection_name) {
        LOG_ERROR("Invalid parameters for optimized_db_query_documents");
        return NULL;
    }
    ...
    if (doc_count > 0) {
        ...
        if (!doc_refs) {
            LOG_ERROR("Failed to allocate memory for document references");
            pthread_mutex_unlock(&db->lock);
            query_free_parse_result(&query_result);
            if (empty_query) {
                json_free(query_json);
...
json_value_t* optimized_db_insert_document(database_t* db, const char* collection_name, json_value_t* document) {
    LOG_INFO("Starting optimized document insertion for collection '%s'", collection_name ? collection_name : "NULL");
    
    if (!db || !collection_name || !document || document->type != JSON_OBJECT) {
        LOG_ERROR("Invalid parameters for optimized_db_insert_document");
        return NULL;
    }
...
json_value_t* optimized_db_get_document(database_t* db, const char* collection_name, const char* id) {
    LOG_INFO("Starting optimized document retrieval for collection '%s', ID '%s'", 
             collection_name ? collection_name : "NULL", 
             id ? id : "NULL");
    
    if (!db || !collection_name || !id) {
        LOG_ERROR("Invalid parameters for optimized_db_get_document");
        return NULL;
    }
