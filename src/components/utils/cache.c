...
cache_t* cache_create(cache_config_t* config) {
  ...
  LOG_INFO("Cache created: %s policy, capacity: %zu entries",
...
void cache_clear(cache_t* cache) {
  ...
  while (entry) {
    ...
    TRACE_DB("Freeing cache entry %zu: key='%s', size=%zu bytes",
  ...
  LOG_INFO("Cache cleared, freed %zu entries", entry_count);
...
void cache_run_cleanup(cache_t* cache) {
  ...
  while (entry) {
    ...
    if (entry->expires > 0 && entry->expires <= now) {
      TRACE_DB("Removing expired entry: key='%s', expired %ld seconds ago",
...
int cache_put(cache_t* cache, const char* key, json_value_t* value, time_t ttl) {
  ...
  if (existing) {
    TRACE_DB("Updating existing cache entry for key '%s'", key);
    ...
    if (cache->config.type == CACHE_TYPE_LRU) {
      TRACE_DB("Moving entry to front (LRU policy).");
      move_to_front(cache, existing);
    } else if (cache->config.type == CACHE_TYPE_LFU) {
      TRACE_DB("Repositioning entry by frequency (LFU policy): count=%u", existing->access_count);
    ...
    TRACE_DB("Creating new cache entry for key '%s'", key);
    ...
    if (!entry) {
      LOG_ERROR("create cache entry for key '%s'", key);
      pthread_mutex_unlock(&cache->lock);
      return 0;
    }
    ...
    if (cache->head) {
      /* Add to front */
      TRACE_DB("Adding entry to front of cache.");
      entry->next = cache->head;
      cache->head->prev = entry;
      cache->head = entry;
    } else {
      /* First entry */
      TRACE_DB("Adding first entry to cache.");
      cache->head = entry;
      cache->tail = entry;
    }
...
json_value_t* cache_get(cache_t* cache, const char* key) {
  ...
  if (entry) {
    ...
    if (entry->expires > 0 && entry->expires <= now) {
      ...
      TRACE_DB("Removing expired entry: key='%s', miss_count=%zu", key, cache->misses);
    ...
    TRACE_DB("Cache hit: key='%s', access_count=%u", key, entry->access_count);
    ...
    if (cache->config.type == CACHE_TYPE_LRU) {
      ...
      TRACE_DB("Repositioning entry by frequency (LFU policy): count=%u", entry->access_count);
    ...
    if (!result) {
      LOG_ERROR("clone value for cache key '%s'", key);
    }
