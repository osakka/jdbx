  ...
  if (count == -1) {
    ...
    if (getcwd(g_binary_dir, PATH_MAX - 1) == NULL) {
      ...
      if (g_logger) {
        ...
        fprintf(stderr, "Warning: Failed to detect binary directory, using default\n");
      }
...
static char* resolve_path(const char* path) {
  ...
  if (!path) {
    if (g_logger) {
      ...
      fprintf(stderr, "Error: NULL path provided to resolve_path()\n");
    }
    return NULL;
  }
...
int config_load_json(const char* filepath, server_config_t* config) {
  ...
  if (file_size <= 0) {
    if (g_logger) {
      ...
      fprintf(stderr, "Warning: Config file is empty or could not determine size: %s\n", filepath);
    }
    fclose(file);
  ...
  if (!json_buffer) {
    if (g_logger) {
      LOG_ERROR("Out of memory", file_size + 1);
  ...
  if (read_size != (size_t)file_size) {
    if (g_logger) {
      LOG_ERROR("read config file: %s (read %zu of %ld bytes)", 
  ...
  if (!json) {
    if (g_logger) {
      LOG_ERROR("parse JSON in config file: %s", filepath);
  ...
  if (g_logger) {
    ...
    printf("Successfully read and parsed JSON configuration file: %s\n", filepath);
  }
      ...
      if (path_val) {
        ...
        if (g_logger) {
          ...
          if (resolved_path) {
            ...
            LOG_ERROR("resolve default database path");
      ...
      if (path_val) {
        ...
        if (g_logger) {
          ...
          if (resolved_path) {
            ...
            LOG_ERROR("resolve default RBAC path");
      ...
      if (file_val) {
        ...
        if (g_logger) {
          ...
          if (resolved_path) {
            ...
            LOG_ERROR("resolve default log file path");
  ...
  if (pid_file_val) {
    if (pid_file_val->type == JSON_STRING) {
      ...
      if (resolved_path) {
        ...
        if (g_logger) {
          LOG_ERROR("resolve default PID file path");
        }
