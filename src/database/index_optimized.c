...
static int resize_index(optimized_index_t* index, size_t new_size) {
    ...
    if (!new_buckets) {
        LOG_ERROR("Failed to allocate memory for index resize");
        return 0;
    }
...
optimized_index_t* db_create_optimized_index(database_t* db, const char* collection, const char* name, 
    ...
    if (!index) {
        pthread_mutex_unlock(&coll->lock);
        LOG_ERROR("Failed to allocate memory for optimized index");
        return NULL;
    }
    ...
    if (!index->buckets) {
        ...
        free(index);
        pthread_mutex_unlock(&coll->lock);
        LOG_ERROR("Failed to allocate memory for index buckets");
        return NULL;
    }
...
json_value_t* db_query_by_optimized_index(database_t* db, const char* collection, const char* field_path, 
    ...
    if (!index) {
        ...
        if (!result) {
            pthread_mutex_unlock(&coll->lock);
            LOG_ERROR("Failed to create result array for collection scan");
            return NULL;
        }
        
        /* Log performance warning */
        LOG_WARNING("Performing full collection scan for query on '%s.%s' - consider creating an index", 
    ...
    if (!result) {
        pthread_rwlock_unlock(&index->lock);
        pthread_mutex_unlock(&coll->lock);
        LOG_ERROR("Failed to create result array for index query");
        return NULL;
    }
...
json_value_t* db_optimized_index_stats(database_t* db, const char* collection, const char* name) {
    if (!db || !collection || !name) {
        LOG_ERROR("Invalid parameters for optimized_index_stats");
        return NULL;
    }
    ...
    if (!stats) {
        pthread_rwlock_unlock(&index->lock);
        pthread_mutex_unlock(&coll->lock);
        LOG_ERROR("Failed to create stats object");
        return NULL;
    }
