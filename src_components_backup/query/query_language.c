...
query_parse_result_t query_parse(json_value_t* query_json) {
    LOG_DEBUG("Parsing query JSON into expression tree");
    ...
    if (!query_json) {
        LOG_ERROR("Failed to parse query: Query JSON is NULL");
        result.error = strdup("Query JSON is NULL");
        return result;
    }

    LOG_TRACE("Query JSON type: %d", query_json->type);
    ...
    if (query_json->type == JSON_OBJECT) {
        ...
        LOG_TRACE("Checking for logical operators at root level");
        json_object_foreach(query_json, qkey, qvalue) {
            field_count++;
            LOG_TRACE("Examining query field: %s", qkey);
        ...
        if (!has_logical_op) {
            ...
            json_object_foreach(query_json, fkey, fvalue) {
                LOG_TRACE("Creating field expression for field: %s", fkey);
    ...
    if (result.error) {
        ...
        LOG_INFO("Query parsed successfully");
    }
...
query_result_t query_execute(query_expr_t* expr, json_value_t* documents, query_options_t* options) {
    ...
    for (int i = 0; i < json_array_size(documents); i++) {
        ...
        if (query_match_document(expr, doc)) {
            ...
            if (matched % 100 == 0) {
                LOG_TRACE("Query progress: %d evaluated, %d matched", evaluated, matched);
            }
        }
    }
    ...
    for (int i = skip; i < max_index; i++) {
        ...
        if (options && options->projection) {
            LOG_TRACE("Applying projection to document at index %d", i);
