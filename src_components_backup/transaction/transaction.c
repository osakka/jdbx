...
transaction_manager_t* transaction_manager_create(database_t* db, int capacity) {
    ...
    if (!manager) {
        LOG_ERROR("Failed to allocate memory for transaction manager");
        return NULL;
    }
    ...
    if (pthread_mutex_init(&manager->lock, NULL) != 0) {
        LOG_ERROR("Failed to initialize transaction manager mutex");
        free(manager);
        return NULL;
    }
    ...
    if (!manager->active_transactions) {
        LOG_ERROR("Failed to allocate memory for active transactions array");
    ...
    if (!manager->tx_hash_table) {
        LOG_ERROR("Failed to allocate memory for transaction hash table");
    ...
    LOG_INFO("Transaction manager created successfully with capacity for %d transactions",
            manager->capacity);
    return manager;
}
...
void transaction_manager_free(transaction_manager_t* manager) {
    ...
    LOG_INFO("Transaction manager freed successfully");
    free(manager);
}
...
transaction_t* transaction_begin(transaction_manager_t* manager, isolation_level_t isolation_level, const char* user_id) {
    if (!manager || !user_id) {
        LOG_ERROR("Failed to begin transaction: Invalid parameters (manager: %p, user_id: %s)",
    ...
    if (!transaction) {
        LOG_ERROR("Failed to allocate memory for transaction structure");
        return NULL;
    }
    ...
    if (!transaction->id) {
        LOG_ERROR("Failed to generate transaction ID");
        free(transaction);
        return NULL;
    }
    ...
    if (!transaction->user_id) {
        LOG_ERROR("Failed to allocate memory for transaction user ID");
        free(transaction->id);
        free(transaction);
        return NULL;
    }
    ...
    if (pthread_mutex_init(&transaction->lock, NULL) != 0) {
        LOG_ERROR("Failed to initialize transaction mutex");
    ...
    LOG_DEBUG("Transaction %s created successfully", transaction->id);
    ...
    if (manager->count >= manager->capacity) {
        ...
        if (!new_array) {
            /* Reallocation failed */
            LOG_ERROR("Failed to resize transaction array to %d elements", new_capacity);
            pthread_mutex_unlock(&manager->lock);
            free(transaction->user_id);
            free(transaction->id);
            free(transaction);
        ...
        manager->capacity = new_capacity;
        LOG_INFO("Transaction array and hash table resized successfully");
    }
    ...
    LOG_INFO("Transaction %s started successfully (isolation: %d, user: %s)",
...
int transaction_commit(transaction_manager_t* manager, transaction_t* transaction) {
    if (!manager || !transaction) {
        LOG_ERROR("Failed to commit transaction: Invalid parameters (manager: %p, transaction: %p)",
                 manager, transaction);
    ...
    LOG_INFO("Transaction %s committed successfully", transaction->id);
    return 0;  /* Success */
}
...
int transaction_rollback(transaction_manager_t* manager, transaction_t* transaction) {
    if (!manager || !transaction) {
        LOG_ERROR("Failed to rollback transaction: Invalid parameters (manager: %p, transaction: %p)",
                 manager, transaction);
        return -1;  /* Invalid transaction */
    }
    ...
    LOG_INFO("Transaction %s rolled back successfully", transaction->id);
    return 0;  /* Success */
}
