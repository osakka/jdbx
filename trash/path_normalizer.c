...
int normalize_config_paths(server_config_t* config, const char* base_dir) {
  ...
  /* Normalize all paths */
  if (config->db_path) {
    config->db_path = normalize_path(config->db_path, base_dir);
  }
  
  if (config->rbac_path) {
    config->rbac_path = normalize_path(config->rbac_path, base_dir);
  }
  
  if (config->pid_file) {
    config->pid_file = normalize_path(config->pid_file, base_dir);
  }
  
  if (config->log_file) {
    config->log_file = normalize_path(config->log_file, base_dir);
  }
  
  if (config->web_root) {
    config->web_root = normalize_path(config->web_root, base_dir);
  }
  
  if (config->validators_dir) {
    config->validators_dir = normalize_path(config->validators_dir, base_dir);
  }
  
  if (config->transforms_dir) {
    config->transforms_dir = normalize_path(config->transforms_dir, base_dir);
  }
  
  if (config->metrics_dir) {
    config->metrics_dir = normalize_path(config->metrics_dir, base_dir);
  }
  
  if (config->cert_path) {
    config->cert_path = normalize_path(config->cert_path, base_dir);
  }
  
  if (config->key_path) {
    config->key_path = normalize_path(config->key_path, base_dir);
  }
