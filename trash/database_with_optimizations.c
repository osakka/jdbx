...
int db_save(database_t* db) {
  ...
  if (!json_str) {
    LOG_ERROR("serialize database collections to JSON");
    pthread_mutex_unlock(&db->lock);
    return 0;
  }
  ...
  if (!file) {
    LOG_ERROR("open database file for writing: %s (error: %s)",
...
int db_load(database_t* db) {
  ...
  if (!file) {
    LOG_ERROR("open database file for reading: %s (error: %s)",
         db->path, strerror(errno));
    pthread_mutex_unlock(&db->lock);
    return 0;
  }
  ...
  if (!collections || collections->type != JSON_OBJECT) {
    LOG_ERROR("parse database file as JSON object");
    if (collections) {
  ...
  LOG_INFO("Database loaded with %zu collections", collection_count);
