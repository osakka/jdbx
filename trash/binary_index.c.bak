...
void* binary_deserialize_index(void* buffer, size_t size) {
    ...
    if (!name) {
        LOG_ERROR("Failed to allocate memory for index name");
        return NULL;
    }
    ...
    if (!field_path) {
        LOG_ERROR("Failed to allocate memory for index field path");
        free(name);
        return NULL;
    }
    ...
    if (!index) {
        LOG_ERROR("Failed to allocate memory for index");
        free(name);
        free(field_path);
        return NULL;
    }
    ...
    if (pthread_rwlock_init(&index->lock, NULL) != 0) {
        LOG_ERROR("Failed to initialize index lock");
        free(name);
        free(field_path);
        free(index->buckets);
        free(index);
    ...
    for (uint32_t i = 0; i < header->entry_count; i++) {
        ...
        if (!key_value) {
            LOG_ERROR("Failed to allocate memory for key value");
            free(doc_id);
            continue;
        }
