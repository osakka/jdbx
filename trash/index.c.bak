...
index_t* db_create_index(database_t* db, const char* collection, const char* name,
    ...
    if (!db || !collection || !name || !field_path) {
        LOG_ERROR("Failed to create index: Invalid parameters (db: %p, collection: %s, name: %s, field_path: %s)",
    ...
    if (!coll) {
        LOG_ERROR("Failed to create index: Collection '%s' not found", collection);
        return NULL;
    }
    ...
    if (!index) {
        LOG_ERROR("Failed to allocate memory for index structure");
        pthread_mutex_unlock(&coll->lock);
        return NULL;
    }
    ...
    if (!index->buckets) {
        LOG_ERROR("Failed to allocate memory for index buckets (%zu buckets)", index->num_buckets);
        free(index->name);
                ...
                if (id_val && id_val->type == JSON_STRING) {
                    ...
                    if (field_val) {
                        ...
                        if (value_str) {
                            ...
                            /* Create new entry */
                            LOG_TRACE("Adding document %s with value '%s' to index", doc_id, value_str);
                            ...
                            if (new_entry) {
                                ...
                                LOG_ERROR("Failed to allocate memory for index entry");
                                free(value_str);
                            }
    ...
    LOG_INFO("Index '%s' created successfully with %zu/%zu documents indexed",
...
int db_drop_index(database_t* db, const char* collection, const char* name) {
    if (!db || !collection || !name) {
        LOG_ERROR("Failed to drop index: Invalid parameters (db: %p, collection: %s, name: %s)",
    ...
    db_collection_t* coll = db_get_collection(db, collection);
    if (!coll) {
        LOG_ERROR("Failed to drop index: Collection '%s' not found", collection);
        return 0;
    }
    ...
    while (current) {
        if (strcmp(current->name, name) == 0) {
            ...
            LOG_INFO("Index '%s' dropped successfully", name);
            return 1;
        }
