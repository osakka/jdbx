...
- `path`: Directory to store metrics files
...
This starts a server at http://localhost:5001/metrics that exposes metrics in Prometheus format.
...
The `/tools` directory includes utilities for analyzing metrics:
...
These tools help identify performance bottlenecks and track changes over time.