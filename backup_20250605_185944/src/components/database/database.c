...
json_value_t* db_insert_document(database_t* db, const char* collection_name, 
    ...
    /* Get or generate ID - check both "uuid" and legacy "_id" and "id" fields */
    char doc_id[256];
    json_value_t* id_field = json_object_get(document, "uuid");
    if (!id_field) {
        id_field = json_object_get(document, "_id"); /* Legacy support */
        if (!id_field) {
    ...
    json_object_set(result_doc, "_id", json_create_string(doc_id));
        ...
        if (doc && doc->type == JSON_OBJECT) {
            ...
            if (matches) {
                ...
                if (result_doc) {
                    ...
                    if (!json_object_get(result_doc, "uuid") && !json_object_get(result_doc, "_id")) {
